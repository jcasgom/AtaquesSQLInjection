...
<head>
	<meta charset="UTF-8">
	<title>High Security Login</title>
		...
		<div class="row">
			<div class="col-lg-10">
				<div class="page-header">
					<h2>Login PHP con Validacion - Dificultad : Alta</h2>
				</div>
...
if(isset($_GET['userID'])){
	...
	if(!$stmt   = $conn->prepare( "SELECT * FROM users WHERE user = ? AND password = ? ;" )){
		echo mysqli_error($conn);
	}
	// Check database
	if(!$stmt->bind_param('ss', $id,$pass)){
		echo mysqli_error($conn);
	}
	if(!$stmt->execute()){
		echo mysqli_error($conn);
